...
<template>
    <div>
        ...
        <div class="card">
            ...
            <KompetensiTable :items="results" :rowsPerPage="rowPerPage" :totalRecords="jmlRows"
                @page-change="handlePageChange" @edit="editItem" @delete="confirmDeleteItem" @search="searchData" />

            <KompetensiFormDialog v-model:visible="formDialog" :item="item" @save="handleSave" :jobfamilys="jobfamilys"
                :subjobfamilys="subjobfamilys" />
            <!-- pake prop juga untuk sub job familynya -->
...
const searchData = async (filterValue) => {
    ...
    let urlParam = globalFilter.value ? `&nama=${globalFilter.value}` : "";
    try {
...
const editItem = async (dataRow) => {
    item.value = { ...dataRow }; // Isi data item dengan data yang dipilih
    await searchDataSubJobFamily(item.value.kode_jf)
    // console.log("=========")
    // console.log(subjobfamilys.value)
    formDialog.value = true;
};
...
const searchDataSubJobFamily = async (kode_jf) => {
    try {
        ...
        subjobfamilys.value = await data.data;
...
const deleteItem = async () => {
    try {
        await custumFetch.delete(`/kompetensis/${itemToDelete.value.id}`, {
            withCredentials: true,
            headers: { "X-API-TOKEN": await getToken() },
        });
...
// Fungsi yang dipanggil saat form disubmit (dari KompetensiFormDialog)
const handleSave = async (itemData) => {
    try {
        if (itemData.id) { // Jika ada ID, lakukan UPDATE
            await custumFetch.put(`/kompetensis/${itemData.id}`, itemData, {
                withCredentials: true,
                headers: { "X-API-TOKEN": await getToken() },
            });
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Kompetensi Updated', life: 3000 });
        } else { // Jika tidak ada ID, lakukan CREATE
            await custumFetch.post("/kompetensis", itemData, {
                withCredentials: true,
                headers: { "X-API-TOKEN": await getToken() },
            });
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Kompetensi Created', life: 3000 });
        }
        searchData();
        ...
        console.error(error);
        toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to save kompetensi', life: 3000 });
    } finally {
    }
};

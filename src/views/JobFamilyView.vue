<template>
    <div>
        ...
        <div class="card">
            ...
            <JobFamilyTable :items="results" :rowsPerPage="rowPerPage" :totalRecords="jmlRows"
                @page-change="handlePageChange" @edit="editItem" @delete="confirmDeleteItem" @search="searchData" />

            <JobFamilyFormDialog v-model:visible="formDialog" :item="item" @save="handleSave" />
...
const deleteItem = async () => {
    try {
        await custumFetch.delete(`/jobfamilys/${itemToDelete.value.id}`, {
            withCredentials: true,
            headers: { "X-API-TOKEN": await getToken() },
        });
...
// Fungsi yang dipanggil saat form disubmit (dari Job_familyFormDialog)
const handleSave = async (itemData) => {
    try {
        if (itemData.id) { // Jika ada ID, lakukan UPDATE
            await custumFetch.put(`/jobfamilys/${itemData.id}`, itemData, {
                withCredentials: true,
                headers: { "X-API-TOKEN": await getToken() },
            });
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Job_family Updated', life: 3000 });
        } else { // Jika tidak ada ID, lakukan CREATE
            await custumFetch.post("/jobfamilys", itemData, {
                withCredentials: true,
                headers: { "X-API-TOKEN": await getToken() },
            });
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Job_family Created', life: 3000 });
        }
        searchData();
    } catch (error) {
        console.error(error);
        toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to save job_family', life: 3000 });
    } finally {

...
<template>
    <div>
        ...
        <div class="card">
            ...
            <MenuTable :items="results" :rowsPerPage="rowPerPage" :totalRecords="jmlRows"
                @page-change="handlePageChange" @edit="editItem" @delete="confirmDeleteItem" @search="searchData" />

            <MenuFormDialog v-model:visible="formDialog" :item="item" @save="handleSave" />
...
const searchData = async (filterValue) => {
    ...
    try {
        const { data } = await custumFetch.get(`/menus/?page=${pageNo.value}&size=${rowPerPage.value}${urlParam}`, {
...
const deleteItem = async () => {
    try {
        await custumFetch.delete(`/menus/${itemToDelete.value.id}`, {
            withCredentials: true,
            headers: { "X-API-TOKEN": await getToken() },
        });

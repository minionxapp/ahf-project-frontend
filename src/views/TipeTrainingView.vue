...
<template>
    <div>
        ...
        <div class="card">
            ...
            <TipeTrainingTable :items="results" :rowsPerPage="rowPerPage" :totalRecords="jmlRows"
                @page-change="handlePageChange" @edit="editItem" @delete="confirmDeleteItem" @search="searchData" />

            <TipeTrainingFormDialog v-model:visible="formDialog" :item="item" @save="handleSave" />
...
const deleteItem = async () => {
    try {
        await custumFetch.delete(`/tipetrainings/${itemToDelete.value.id}`, {
            withCredentials: true,
            headers: { "X-API-TOKEN": await getToken() },
        });
...
// Fungsi yang dipanggil saat form disubmit (dari Tipe_trainingFormDialog)
const handleSave = async (itemData) => {
    try {
        if (itemData.id) { // Jika ada ID, lakukan UPDATE
            await custumFetch.put(`/tipetrainings/${itemData.id}`, itemData, {
                withCredentials: true,
                headers: { "X-API-TOKEN": await getToken() },
            });
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Tipe_training Updated', life: 3000 });
        } else { // Jika tidak ada ID, lakukan CREATE
            await custumFetch.post("/tipetrainings", itemData, {
                withCredentials: true,
                headers: { "X-API-TOKEN": await getToken() },
            });
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Tipe_training Created', life: 3000 });
        }
        searchData();
        ...
        console.error(error);
        toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to save tipe_training', life: 3000 });
    } finally {
    }
};

...
<template>
    <div>
        ...
        <div class="card">
            ...
            <LevelTable :items="results" :rowsPerPage="rowPerPage" :totalRecords="jmlRows"
                @page-change="handlePageChange" @edit="editItem" @delete="confirmDeleteItem" @search="searchData" />

            <LevelFormDialog v-model:visible="formDialog" :item="item" @save="handleSave" />
...
const searchData = async (filterValue) => {
    ...
    try {
        const { data } = await custumFetch.get(`/levels/?page=${pageNo.value}&size=${rowPerPage.value}${urlParam}`, {
...
const deleteItem = async () => {
    try {
        await custumFetch.delete(`/levels/${itemToDelete.value.id}`, {
            withCredentials: true,
            headers: { "X-API-TOKEN": await getToken() },
        });
        toast.add({ severity: 'success', summary: 'Successful', detail: 'Level Deleted', life: 3000 });
        searchData();
    } catch (error) {
        console.error(error);
    } finally {
...
// Fungsi yang dipanggil saat form disubmit (dari LevelFormDialog)
const handleSave = async (itemData) => {
    try {
        if (itemData.id) { // Jika ada ID, lakukan UPDATE
            await custumFetch.put(`/levels/${itemData.id}`, itemData, {
                withCredentials: true,
                headers: { "X-API-TOKEN": await getToken() },
            });
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Level Updated', life: 3000 });
        } else { // Jika tidak ada ID, lakukan CREATE
            await custumFetch.post("/levels", itemData, {
                withCredentials: true,
                headers: { "X-API-TOKEN": await getToken() },
            });
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Level Created', life: 3000 });
        }
        searchData();
        ...
        console.error(error);
        toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to save level', life: 3000 });
    } finally {
    }
};
